...
<form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">    
    <div class="buttons">
        ...
        <a href="/admin/users" style="float:left" class="button"><span><?php echo lang('cancel'); ?></span></a>
  ...
  <div class="content">
    ...
    ?>       
        <tr>
          ...
          <td>
              <select name="id_auth_group">
              <?php
              foreach($users_group as $item)
              {
                  echo "<option value=$item[id_auth_group] ";
                  //if(isset($user['id_auth_group']) and $user['id_auth_group']==$item['id_auth_group']) echo " selected=selected ";
                  if(isset($user['id_auth_group']) and ($user['id_auth_group']==$item['id_auth_group'])) echo set_select('id_auth_group',$user["id_auth_group"],TRUE,1);
                  else echo set_select('id_auth_group',$item["id_auth_group"]);
                   //((isset($user['is_active'])=="")?set_select('status',1,TRUE):set_select('status',$user["is_active"]))
        ...
        <tr>
          ...
          <td>
              <a href="/admin/users_form/<?=$user['id']?>/1/1"><?=lang('change_pwd_now')?></a>
          </td>
        </tr>        
        <?php
        }
