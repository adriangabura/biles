...
<div class="box">
  ...
  <div class="right"></div>
  <div class="heading">
    <h1 style="background-image: url('/img/product.png');"><?=lang('zona')?></h1>
<form action="<?=$action; ?>" onsubmit="return check_field('<?=lang('input_request')?>','<?=lang('pwd_request')?>')" method="post" enctype="multipart/form-data" id="form">    
    <div class="buttons">
        ...
        <a href="/admin/zona" class="button"><span><?=lang('cancel')?></span></a>
    </div>
  </div>
  <div class="content">    
	   ...
	   <?php } ?>
        ...
        <?php foreach ($langs as $lang) { ?>        
         <div>
          <table class="form">
            <tr>
              <td><span class="required">*</span> Zona</td>
              <td>
                 <input name="page[<?=$lang['id_langs']?>][title_<?=$lang['ext']?>]" value="<?=((isset($page['title_'.$lang['ext']])=="")?"":$page['title_'.$lang['ext']])?>"   size="100" value="" />                
              </td>
            </tr>           
            <tr>
              <td><span class="required">*</span> Subzona</td>
              <td>
                 <input name="page[<?=$lang['id_langs']?>][subzona]" value="<?=((isset($page['subzona'])=="")?"":$page['subzona'])?>"   size="100" value="" />                
              </td>
            </tr>                  
          </table>
         </div>            
